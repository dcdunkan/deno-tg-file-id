...
export class Util {
  ...
  private static rleDecode(input: Buffer) {
    let last = "",
      news = "";
    const splited = input.toString("binary");
    for (const cur of splited) {
      if (last === "\0") {
  ...
  private static readTLString(data: Buffer) {
    ...
    if (l === 254) {
      ...
      tmpBuff.write("\0", 3);
      const long_len = tmpBuff.readInt32LE();
      x = data.slice(4, 4 + long_len);
      const resto = Util.posmod(-long_len, 4);
      if (resto > 0) {
  ...
  static packTLString(input: Buffer) {
    ...
    if (l <= 253) {
      ...
      buff.writeInt32LE(l);
      const nodeOut = buff.slice(0, 3);
      output += nodeOut.toString();
      output += input;

      const t2 = "\0".repeat(Util.posmod(-l, 4));
      output += t2;
    }
  ...
  static decodeFileId(fileId: string): FileIdInfo {
    const base64Decoded = Util.base64UrlDecode(fileId);
    let rlDecoded = Util.rleDecode(base64Decoded);
    const out: FileIdInfo = {} as FileIdInfo;
    ...
    if (out.hasReference) {
      const { x, newData } = Util.readTLString(rlDecoded);
      rlDecoded = newData;
      out.fileReference = x.toString("hex");
    }
    if (out.hasWebLocation) {
      const { x } = Util.readTLString(rlDecoded);
      out.url = x.toString();
    ...
    if (out.typeId <= 2) {
      ...
      switch (out.photoSizeSource) {
        ...
        case Util.PHOTOSIZE_SOURCE_THUMBNAIL: {
          const typeId = rlDecoded.readUInt32LE(12);
          out.fileType = Util.TYPES[typeId];
          out.thumbTypeId = typeId;
          out.thumbnailType = rlDecoded.slice(16, 20).toString().replace(
        ...
        case Util.PHOTOSIZE_SOURCE_DIALOGPHOTO_SMALL:
          out.photoSize =
            out.photoSizeSource === Util.PHOTOSIZE_SOURCE_DIALOGPHOTO_SMALL
              ? "small"
              : "big";
  ...
  static decodeUniqueFileId(input: string) {
    ...
    if (out.typeId === Util.UNIQUE_WEB) {
      const { x } = Util.readTLString(rlDecoded.slice());
      out.url = x.toString();
...
export const decodeUniqueFileId = Util.decodeUniqueFileId;
